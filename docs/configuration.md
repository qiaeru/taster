...
| `TRUST_PROXY` | `0` | Reverse proxies whose `X-Forwarded-*` headers are honored. `1` trusts a proxy on localhost or a private network (the usual Caddy/Traefik/nginx on the same host or Docker network); otherwise list the proxy IPs or CIDRs, comma separated, or the presets `loopback`, `linklocal`, `uniquelocal`. Only headers appended by those addresses are trusted, so clients cannot spoof their IP to the rate limits and login lockout; `0` means no proxy. |
