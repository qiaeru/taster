...
- **Per-IP login lockout**: failed logins are counted per source IP (in memory, reset on restart), so an attacker only ever blocks their own address and can never lock the admin out. The flip side is that an attacker rotating IPs is throttled only by the per-IP rate limit; with the mandatory 12+ character zxcvbn-checked password and the Argon2id verification cost, an online brute force remains impractical. Behind a reverse proxy, set `TRUST_PROXY` to the exact number of proxies (usually `1`) so the real client IP is used rather than the proxy's; the count also caps how much of `X-Forwarded-For` is trusted, so a client cannot spoof an arbitrary IP (or the admin's) to the limits. The app port should only be reachable through the proxy whenever `TRUST_PROXY` is set.
