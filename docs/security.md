...
- **Per-IP login lockout**: failed logins are counted per source IP (in memory, reset on restart), so an attacker only ever blocks their own address and can never lock the admin out. The flip side is that an attacker rotating IPs is throttled only by the per-IP rate limit; with the mandatory 12+ character zxcvbn-checked password and the Argon2id verification cost, an online brute force remains impractical. Behind a reverse proxy, set `TRUST_PROXY` to the proxy addresses (`1` for a proxy on localhost or a private network, the case of the bundled Docker stacks) so the real client IP is used rather than the proxy's; `X-Forwarded-For` is only trusted as far as those addresses go, so a client cannot spoof an arbitrary IP (or the admin's) to the limits. Hop counts are no longer accepted, because they never checked who was actually connecting. The app port should only be reachable through the proxy whenever `TRUST_PROXY` is set: with `1`, any machine on the same private network could otherwise forge the headers.
