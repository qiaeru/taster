...
- **Images**: card thumbnails are decorative (`alt=""`) since the adjacent title carries the information; the detail hero uses the optional cover image description as alt when the admin filled one, falling back to the taste title. Fixed width/height attributes avoid layout shifts that disorient magnifier users.
...
- **Motion**: page changes use a short cross-fade (View Transitions where supported), with subtle touches like a loading-skeleton shimmer, a die roll on the random button, and the decorative floating icons (`aria-hidden`) behind the admin sign-in. Everything is disabled under `prefers-reduced-motion: reduce`, and public pages have no autoplaying or parallax motion.
